...
public class RTEditText extends EditText implements TextWatcher, SpanWatcher, LinkSpanListener {
    ...
    /**
     * Needs to be called if a media is added to the editor. Important to be able to delete obsolete media once we're
     * done editing.
     */
    ...
    /**
     * Find the start and end of the paragraph(s) encompassing the current selection. A paragraph spans from one \n
    ...
    /**
     * This method returns the Selection which makes sure that selection start is <= selection end. Note:
     * getSelectionStart()/getSelectionEnd() refer to the order in which text was selected.
     */
    ...
    /**
     * Sets the edit mode to plain or rich text. The text will be converted automatically to rich/plain text if
     * autoConvert is True.
     *
     * @param useRTFormatting True if the edit mode should be rich text, False if the edit mode should be plain text
     * @param autoConvert     Automatically convert the content to plain or rich text if this is True
     */
    ...
    /**
     * Sets the edit mode to plain or rich text and updates the content at the same time. The caller needs to make sure
     * the content matches the correct format (if you pass in html code as plain text the editor will show the html
     * code).
     *
     * @param useRTFormatting True if the edit mode should be rich text, False if the edit mode should be plain text
     ...
     * <p/>
     * It will convert the text from rich text to plain text if the editor's mode is set to use plain text. or to a
     * spanned text (only supported formatting) if the editor's mode is set to use rich text
     * <p/>
     * We need to prevent onSelectionChanged() to do anything as long as setText() hasn't finished because the Layout
     * doesn't seem to update before setText has finished but onSelectionChanged will still be called during setText and
     * will receive the out-dated Layout which doesn't allow us to apply styles and such.
     */
    ...
    /**
     * Returns the content of this editor as a String. The caller is responsible to call only formats that are supported
     * by RTEditable (which is the rich text editor's format and always the source format).
     *
...
//                    "The RTMediaFactory is null. Please make sure to register the editor at the RTManager before using it.");
        }
    }
    ...
    /**
     * Ignore changes that would trigger a RTEditTextListener.onTextChanged() method call. We need this during the
     * actual undo/redo operation (or an undo would create a change event itself).
     */
    ...
    /**
     * If changes happen call RTEditTextListener.onTextChanged(). This is needed for the undo/redo functionality.
     */
    ...
    public synchronized void afterTextChanged(Editable s) {
        ...
        if (mListener != null && !mIgnoreTextChanges && !newText.equals(theText)) {
            ...
            mListener.onTextChanged(this, mOldSpannable, newSpannable, mSelStartBefore, mSelEndBefore,
    ...
    /**
     * <p>This is where the "magic" happens.</p> <p>The menu used to cut/copy/paste is a normal ContextMenu, which
     * allows us to overwrite the consuming method and react on the different events.</p>
     *
     * @see <a href="http://grepcode.com/file/repository.grepcode.com/java/ext/com.google.android/android/2.3_r1/android/widget/TextView.java#TextView.onTextContextMenuItem%28int%29">Original
     * Implementation</a>
     */
    @Override
    ...
    public void onTextPaste() {
        Whitelist whitelist = Whitelist.none().addTags("br", "b", "i", "u");
        String    jsoupHtml = Jsoup.clean(getText(RTFormat.HTML), whitelist);
        setRichTextEditing(true, jsoupHtml);
    }
    ...
    /**
     * Call this to have an effect applied to the current selection. You get the Effect object via the static data
     * members (e.g., RTEditText.BOLD). The value for most effects is a Boolean, indicating whether to add or remove the
     * effect.
     */
