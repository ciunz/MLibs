...
import com.iapps.libs.generics.BaseStepFragment;
...
public class BaseHelper {
    ...
    //        final GridLayoutManager mLayoutManager = new WrapContentLinearLayoutManager(adapter.getContext(), adapter.getNumGrid(),
...
//                .size(new OriginalSize())
                ...
                .subscribe(
                        new Action1<Response<GenericFragment, String>>() {
                            @Override
                            public void call(
                                    final Response<GenericFragment, String> response) {
...
//                .size(new OriginalSize())
                ...
                .subscribe(
                        new Action1<Response<GenericFragment, String>>() {
                            @Override
                            public void call(
                                    Response<GenericFragment, String> response) {
                                if (response.resultCode() != Activity.RESULT_OK) {
                                    response.targetUI().imagePickFail();
                                    return;
                                }
    ...
    public static void picGalleryCustomSize(final BaseStepFragment fragment, final ImageView imageView,
                                            final BaseHelper.ImagePickDone listenerDone, final int x, final int y) {
        Options options = new Options();
        options.setMaxResultSize(x, y);
        options.setCompressionFormat(Bitmap.CompressFormat.JPEG);
...
//                .size(new OriginalSize())
                ...
                .usingGallery()
                .subscribe(
                        new Action1<Response<BaseStepFragment, String>>() {
                            @Override
                            public void call(
                                    Response<BaseStepFragment, String> response) {
                                if (response.resultCode() != Activity.RESULT_OK) {
                                    response.targetUI().imagePickFail();
                                    return;
                                }
...
//                    BaseHelper.showRequired(editText.getContext(), "");
                    showAlert(editText.getContext(), "Some Field Required", "", null);
