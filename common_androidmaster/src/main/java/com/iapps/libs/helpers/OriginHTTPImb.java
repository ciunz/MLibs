 ...
 */
public abstract class OriginHTTPImb extends HTTPAsyncTask {
    ...
    public OriginHTTPImb(Activity activity, boolean displayProgress) {
    ...
    public OriginHTTPImb(Activity activity, LoadingCompound ld) {
        this.activity = activity;
        this.ld = ld;
    ...
    public OriginHTTPImb(Fragment frag, boolean displayProgress) {
        this.fragment = frag;
        this.displayProgress = displayProgress;
    ...
    public OriginHTTPImb(Fragment frag, LoadingCompound ld) {
        this.fragment = frag;
        this.ld = ld;
        setDefaultValue();
    }
    ...
    protected void onPostExecute(Response response) {
        ...
        if (!onSuccessRaw(response)) {
            ...
            if (json != null) {
//                ganti jadi optInt supaya ga perlu pake try catch dan cuma bisa salah satu 3 nov
//                try {
//                    if (json.getInt(STATUS_CODE) == 1) {
//                        if (ld != null)
//                            ld.hide();
//                        onSuccess(json);
//                    } else {
...
//                }
                if (json.optInt(STATUS_CODE, -1) == 1) {
                    ...
                    onFail(CODE_INVALID_RESPONSE, ERR_NO_STATUS_CODE);
                } else {
                    onFail(CODE_BACKEND_FAIL, json);
                }
            } else {
    ...
    protected JSONObject handleResponse(Response response, boolean shouldDisplayDialog, Context context) {
        if (response != null) {
